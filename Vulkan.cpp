...
void StartGLFW()
{
	...
	GLFWimage images[1]; 
	images[0].pixels = stbi_load("vulkan.png", &images[0].width, &images[0].height, 0, 4);
...
void CreateImageViews()
{
	...
	{
		CreateImageView(device, swapchainImages[i], BGRA, VK_IMAGE_ASPECT_COLOR_BIT, imageViews[i]);
	}
...
void CreateRenderPass()
{
	...
	VkAttachmentDescription DepthAttachment = GetDepthAttachment(physicalDevices[0]);
...
void CreatePipeline()
{
	...
	RasterCreateInfo.cullMode = VK_CULL_MODE_NONE;			//VK_CULL_MODE_NONE	VK_CULL_MODE_BACK_BIT
	...
	VkPipelineDepthStencilStateCreateInfo DepthStencilStateCreateInfo = GetDepthStencilStateCreateInfoOpaque();
...
void UpdateMVP()
{
	...
	glm::mat4 Model = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, 0.0f));
			  Model = glm::rotate(Model, TimeSinceStart * glm::radians(0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
			  Model = glm::rotate(Model, TimeSinceStart * glm::radians(0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
			  Model = glm::rotate(Model, TimeSinceStart * glm::radians(0.0f), glm::vec3(1.0f, 0.0f, 0.0f));
...
void GameLoop()
{
	...
	while(!glfwWindowShouldClose(window))
	{
		...
		Rotate(glfwGetTime() * 60.0, 0.0, 0.0, 1.0);
		Rotate(glfwGetTime() * 80.0, 1.0, 0.0, 0.0);
		Rotate(glfwGetTime() * 70.0, 0.0, 1.0, 0.0);
		Scale(0.5, 0.5, 0.5);
		PushMatrix();
